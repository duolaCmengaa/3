# 找到了最好的学习率和batch size,接下来对weight_decay和momentum进行查找,经过运行train2后，我找到了学习率为..batch size为...在验证集上的准确率最高
...
if __name__ == '__main__':
    ...
    thirdmodepth = os.path.join(model_path, 'firstmodepth_3.pth')
    log_dir = os.path.join(result_dir, 'pre_log1')
    best_log_dir = os.path.join(result_dir, 'best_pre_log1')
    picture_dir = os.path.join(result_dir, 'pirtures')
    os.makedirs(picture_dir, exist_ok=True)

    picture_save_path = os.path.join(picture_dir, "train2_weight_decay_momentum")
    ...
    '''
    '''
    wds = [0.008,0.006,5e-3, 2e-3,1e-3, 5e-4, 1e-4]
    moms = [0.85,0.9,0.95,0.98,0.99]
    '''
    wds = [0.008,0.0085,0.0075,0.009]
    moms = [0.84,0.8,0.855]
    for mom in moms:
        best_acc_list = []
                ...
                for epoch in range(epochs):
                    ...
                    if(val_accurate > best_acc):
                        ...
                        if val_accurate > total_best_acc:
                            ...
                            best_mom = mom
                            tmp=1
                            torch.save(net.state_dict(), thirdmodepth) # 保存模型
    ...
    plt.xticks(x, wds)
    # 添加标题
    plt.title('train2 weight decay and momentum')
    # 添加图例
    plt.legend()
